...
test('同一工作台可以叠加多个对象并使用不同的柔和颜色', () => {
  ...
  assert.equal(first.equalAspect, true)
  assert.deepEqual([first.xLabel, first.yLabel], ['x', 'y'])
  assert.equal(buildPlotData(first)[0].name, first.name)
  assert.equal(buildPlotData(second)[0].name, second.name)
})

test('定义域支持无穷区间和间断区间', () => {
  const intervals = parseDomain('(-∞, 0) ∪ (1, ∞)')
  assert.equal(intervals.length, 2)
  assert.deepEqual(domainViewRange('[0, 2π]'), [0, 2 * Math.PI])
  const object = objectFromExample({ id: 'domain', name: '间断函数', type: 'cartesian2d', expression: 'x', range: { x: [-3, 3] }, domain: '(-∞, 0) ∪ (1, ∞)' })
  assert.ok(buildPlotData(object)[0].x.includes(null))
})
